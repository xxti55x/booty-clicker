...
describe('ch-state — Crew-Umschulung im derived layer (3b)', () => {
  /** Booty-Boss (Rhythmus P S P S, Stock `critdmg`) mit vier gekauften Stufen. */
  const base = {
    ...createChState(),
    crew: { boss: 200, producer: 200 },
    crewUp: { boss: 4 },
  };
  ...
  it('faltet die Stock-Sorten ohne Map (jeder Aufrufer vor 3b bleibt zahlengleich)', () => {
    const { crewRetrain: _drop, ...withoutSlice } = base;
  ...
  it('ist permanent: die Sorten überleben alle drei Resets, der Eskalator nicht', () => {
    const s = {
      ...base,
      crewRetrain: { boss: { '2': 'idle' as const } },
      retrainRolls: { boss: 3 },
      runMaxZone: 60,
      lifetimeMaxZone: 60,
      rsLifetime: 1_500,
    ...
    for (const next of [ascendState(s), himmelfahrtState(s), transcendState(s)]) {
      // Die erkaufte Sorte bleibt — sonst wäre der Splitter-Einsatz nach der
      // nächsten Aszension verpufft …
      expect(next.crewRetrain).toEqual({ boss: { '2': 'idle' } });
      // … der Kosten-Eskalator dagegen ist Run-Zustand und startet neu.
      expect(next.retrainRolls).toEqual({});
      // Die Slots selbst fallen mit dem Ledger und werden neu gekauft.
      expect(next.crewUp).toEqual({});
    }
  });
});
